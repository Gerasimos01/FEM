...
namespace MGroup.FEM.Structural.Shells
{
	...
	/// </summary>
	public class Shell8NonLinear : IStructuralElementType
	{
		...
		private void CalculateInitialConfigurationData(out double[][] tx_i)
		{
			double[] E;
			double[] ni;
			...
			for (int j = 0; j < 8; j++)
			{
				tx_i[j] = new double[] { Nodes[j].X, Nodes[j].Y, Nodes[j].Z, };
			...
			(Matrix[] J_0inv, double[] detJ_0) =
				JacobianShell8Calculations.GetJ_0invAndDetJ_0(J_0a, Nodes, oVn_i, nGaussPoints);
		...
		private void CalculateStrains(double[][] tx_i)
		{
			IReadOnlyList<double[]> shapeFunctions = Interpolation.EvaluateFunctionsAtGaussPoints(QuadratureForStiffness);
			IReadOnlyList<Matrix> shapeFunctionsDerivatives = Interpolation.EvaluateNaturalGradientsAtGaussPoints(QuadratureForStiffness);
		...
		{
			UpdateCoordinateData(localTotalDisplacements, out double[][] tx_i);
			CalculateStrains(tx_i);
		...
		public virtual IMatrix StiffnessMatrix()
		{
			...
			if (stiffnessCase == 1)
			{
				CalculateInitialConfigurationData( out double[][] tx_i);
				CalculateStrains( tx_i);
