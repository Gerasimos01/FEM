...
namespace MGroup.FEM.Structural.Embedding
{
	...
	/// </summary>
	public class CohesiveShell8ToHexa20 : IStructuralElementType, IEmbeddedElement
	{
		...
		private void GetInitialGeometricDataForMidsurface() 
		{
			ox_i_shell_midsurface = new double[8][];
			for (int j = 0; j < 8; j++)
			{
				ox_i_shell_midsurface[j] = new double[] { Nodes[j].X, Nodes[j].Y, Nodes[j].Z, };
			}
		}
		...
		private void GetInitialGeometricDataAndInitializeMatrices() 
		{
			...
			this.GetInitialGeometricDataForMidsurface();
			ox_i = new double[16][];
			...
			{
				for (int j = 0; j < 8; j++)
				{ ox_i[j] = new double[] { Nodes[j + 8].X, Nodes[j + 8].Y, Nodes[j + 8].Z, }; }
				for (int j = 8; j < 16; j++)
				{
		...
		private double[,] CalculateTMatrix()
		{
			double[,] T;
			T = new double[24, 40];
		...
		private double[] MultiplyForcesForEmbedding(double[] fxk1_coh)
		{
			double[,] T = CalculateTMatrix();
		...
		private double[,] MultiplyStifnessMatrixForEmbedding(double[,] k_element_coh)
		{
			double[,] T = CalculateTMatrix();
		...
		private double[] UpdateResponseIntegral(Matrix[] RtN3, double[] integrationCoeffs)
		{
			double[] fxk2_coh = new double[64];
			...
			fxk2_coh = this.MultiplyForcesForEmbedding(fxk1_coh);
			return fxk2_coh;
		}
		...
		private double[,] UpdateKmatrices(Matrix[] RtN3, double[] integrationCoeffs)
		{
			...
			k_element_coh2 = this.MultiplyStifnessMatrixForEmbedding(k_stoixeiou_coh);
			return k_element_coh2;
		}
		...
		public double[] CalculateResponseIntegral()
		{
			...
			fxk2_coh = this.UpdateResponseIntegral(RtN3, integrationCoeffs);
